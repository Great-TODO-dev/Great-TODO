...
class Repository {
  ...
  static const DBTABLEPARAMS = '$TASKSTABLENAME(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, description TEXT, date TEXT, deadline TEXT, tags TEXT, store TEXT)';

  static Future<sql.Database> getInstance() async {
    final dbPath = await sql.getDatabasesPath();
    final db = await sql.openDatabase(path.join(dbPath, DBNAME),
    onOpen: (db) {db.execute('CREATE TABLE IF NOT EXISTS $DBTABLEPARAMS');},
        onCreate: (db, version) {
      return db.execute(
          'CREATE TABLE $DBTABLEPARAMS');
    }, version: 1);
    return db;
  }
  ...
  static Future<void> insertTask(Task task) async {
    ...
    db.insert(TASKSTABLENAME, {
      ...
      'deadline': task.deadline == null ? 'null' : task.deadline.toIso8601String(),
